...
Посилання: https://www.google.com.ua/''')
    ...
    def save(
        ...
        group_chat = Chat.objects.filter(type=Chat.ChatTypes.DIPLOMA, group=self)
        if not group_chat:
            group_chat = Chat.objects.create(name=f'Дипломний чат {self.name}', type=Chat.ChatTypes.DIPLOMA, group=self)
